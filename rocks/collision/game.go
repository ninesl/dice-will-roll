...
const (
	...
	defaultRocksPerSize = 10
)

type game struct {
	rocks       rocks.Rocks
	rockAtlas   *rocks.RockSpriteAtlas
	rockLayers  *rocks.RockLayers
	amountScale int
...
func Run() {
	...
	rockState.SortByMortonPosition()
	...
	if err := ebiten.RunGame(&game{
		rocks:          rockState,
		rockAtlas:      rockAtlas,
		rockLayers:     rocks.NewRockLayers(settings.Screen.ResolutionX, settings.Screen.ResolutionY),
...
func (g *game) removeRockSet() {
	rockCount := max(0, g.rocks.Len()-g.rocksPerSet)
	g.rocks.PosX = g.rocks.PosX[:rockCount]
	g.rocks.PosY = g.rocks.PosY[:rockCount]
	g.rocks.Slope = g.rocks.Slope[:rockCount]
	g.rocks.Stepping = g.rocks.Stepping[:rockCount]
	g.catchUpRockState()
}

func (g *game) catchUpRockState() {
	g.rocks.SortByMortonPosition()
	g.amountScale = rocks.RockAmountScaleIndex(g.rocks.Len())
	g.rocks.InvalidateLayers()
	if g.rocks.Len() == 0 {
		g.rockLayers.Composite().Clear()
	}
}
...
func (g *game) Update() error {
	...
	if hasChanges(g.debugInput) {
		...
		if g.debugInput.IncrementAmountScale || g.debugInput.DecrementAmountScale ||
			g.debugInput.IncreaseSizeScale || g.debugInput.DecrementSizeScale {
			g.rocks.SortByMortonPosition()
		}
	}
	...
	g.rockLayers.Update(
		&g.rocks, g.rockAtlas, g.amountScale,
		settings.Screen.DrawOptions.DrawImageOptions)
	g.updateTick++
...
func (g *game) Draw(screen *ebiten.Image) {
	compositeOptions := ebiten.DrawImageOptions{Blend: ebiten.BlendCopy}
	screen.DrawImage(g.rockLayers.Composite(), &compositeOptions)
	if g.debugTextImage == nil {
		return
	}
	settings.Screen.DrawOptions.GeoM.Reset()
	settings.Screen.DrawOptions.GeoM.Translate(settings.Screen.FontSize-1, settings.Screen.FontSize-1)
	settings.Screen.DrawOptions.ColorScale.Reset()
	screen.DrawImage(g.debugTextImage, &settings.Screen.DrawOptions.DrawImageOptions)
}
...
func (g *game) updateDebugText(debug *rocks.RockDebug) {
	...
	width, height := text.Measure(debugText, g.fontFace, settings.Screen.DrawOptions.LineSpacing)
	g.debugTextImage = ebiten.NewImage(int(math.Ceil(width))+2, int(math.Ceil(height))+2)

	for _, offset := range [4][2]float64{{0, 1}, {2, 1}, {1, 0}, {1, 2}} {
		settings.Screen.DrawOptions.GeoM.Reset()
		settings.Screen.DrawOptions.GeoM.Translate(offset[0], offset[1])
		settings.Screen.DrawOptions.ColorScale.Reset()
		settings.Screen.DrawOptions.ColorScale.SetWithColor(color.Black)
		text.Draw(g.debugTextImage, debugText, g.fontFace, settings.Screen.DrawOptions)
	}
	settings.Screen.DrawOptions.GeoM.Reset()
	settings.Screen.DrawOptions.GeoM.Translate(1, 1)
	settings.Screen.DrawOptions.ColorScale.Reset()
